        ...
        <div id="box-1">
            ...
            <div id="box-last">
                <div id="box-14">
                    ...
                    <button @click="calculateDividedDifferences">Calculate</button>
                </div>
                <!-- Output Section -->
                <div id="box-2" v-if="coefficients.length > 0">
                    <h4>Divided Difference Coefficients:</h4>
                    <ul>
                        <li v-for="(coef, index) in coefficients" :key="'coef-' + index">
                            a{{ index }} = {{ coef }}
                        </li>
                    </ul>
                </div>
...
<script>
export default {
    name: "NewtonsDividedDifferences",
    data() {
        return {
            numPoints: 2,
            dataPoints: [],
    ...
    methods: {
        ...
        calculateDividedDifferences() {
            try {
                ...
                let table = this.dataPoints.map(point => [point.y]);
                for (let j = 1; j < n; j++) {
                    for (let i = 0; i < n - j; i++) {
                        const numerator = table[i + 1][j - 1] - table[i][j - 1];
                        const denominator = this.dataPoints[i + j].x - this.dataPoints[i].x;
                        table[i].push(numerator / denominator);
                    }
                }
                this.coefficients = table[0];
            } catch (error) {
                console.error("Error calculating Newton's Divided Differences:", error);
